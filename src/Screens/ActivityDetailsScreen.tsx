...
function ActivityDetailsScreen({ navigation }) {
  ...
  const [latestPrediction, setLatestPrediction] = useState('Mopping');
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        setLatestId(maxId);
        if (data.length > 0) {
          setLatestPrediction(data[0].prediction); // Assuming 'activity' is the field for prediction
        }
      } catch (error) {
      ...
      <View style={{ width: '100%', marginLeft: 12, marginTop: 20 }}>
        ...
        <View style={{ marginLeft: 80, flexDirection: 'row', marginTop: 60 }}>
          ...
          <View style={styles.textContainer}>
            <Text style={styles.itemName}>Abdullah</Text>
            <View
              style={[styles.roleContainer, {
                backgroundColor: getStateColor(latestPrediction),
                paddingHorizontal: latestPrediction ? (latestPrediction.length < 6 ? 12 : 8) : 12,
              }]}
            >
              <Text style={[styles.itemRole, { color: getStateTextColor(latestPrediction) }]}>{latestPrediction}</Text>
