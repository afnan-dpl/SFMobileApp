...
function WorkerListScreen({ navigation }) {
    ...
    const fetchWorkers = async () => {
      try {
        ...
        workerData.forEach(worker => {
          ...
          if (prediction) {
            const predictionTime = moment.utc(prediction.start_time);
            const timeDifference = currentTime.diff(predictionTime, 'minutes');
            console.log("timeDifference is ", predictionTime.format(), currentTime.format(), timeDifference);
            predictionMap[worker.user_id] = timeDifference > 55 ? 'Idle' : prediction.prediction;
          } else {
  ...
  const renderItem = ({ item, index }: { item: WorkerType, index: number }) => (
    <TouchableOpacity onPress={() => navigation.navigate('ActivityDetailsScreen', { userId: item.user_id })}>
